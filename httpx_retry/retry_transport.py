...
class RetryTransport(httpx.AsyncBaseTransport, httpx.BaseTransport):
    ...
    Modified from https://github.com/encode/httpx/issues/108#issuecomment-1434439481
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        """
        transport: httpx.AsyncBaseTransport = cast(httpx.AsyncBaseTransport, self._wrapped_transport)
        if request.method in self._retryable_methods:
    ...
    async def aclose(self) -> None:
        ...
        """
        transport = cast(httpx.AsyncBaseTransport, self._wrapped_transport)
    ...
    def _calculate_sleep(self, attempts_made: int, headers: httpx.Headers | Mapping[str, str]) -> float:
        ...
        full_jittered_backoff = backoff * random.random()
        return min(full_jittered_backoff, self._max_backoff_wait)
    ...
    async def _retry_operation_async(
        ...
        while True:
            if response is not None:
                await asyncio.sleep(self._calculate_sleep(attempts_made, response.headers))
